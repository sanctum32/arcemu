...
#ifdef WIN32
			for(std::unordered_map<string, Account*>::iterator itr = AccountDatabase.begin(); itr != AccountDatabase.end(); ++itr)
#else
...
#ifdef WIN32
			std::unordered_map<string, Account*>::iterator itr = AccountDatabase.find(Name);
#else
...
#ifdef WIN32
		std::unordered_map<string, Account*> AccountDatabase;
#else
