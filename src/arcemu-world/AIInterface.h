...
typedef std::unordered_map<Unit*, int32> TargetMap;
#else
...
typedef std::unordered_map<Unit*, int32, HM_NAMESPACE::hash<Unit*> > TargetMap;
#endif
*/
...
//	Unit *gracefull_hit_on_target;
	...
	public:
		...
		void MoveJump(float x, float y, float z, float o = 0.0f);
		void MoveJump(float x, float y, float z, float horizontal, float vertical, float o = 0.0f);
