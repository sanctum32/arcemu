...
{
	std::unordered_map<uint32, list<QuestRelation*>* > &olist = _GetList<Creature>();
	std::unordered_map<uint32, QuestRelationList* >::iterator itr = olist.find(entryid);
	return (itr == olist.end()) ? 0 : itr->second;
}
...
{
	std::unordered_map<uint32, list<QuestRelation*>* > &olist = _GetList<T>();
	...
	if(olist.find(entryid) == olist.end())
	{
		...
		olist.insert(std::unordered_map<uint32, list<QuestRelation*>* >::value_type(entryid, nlist));
	}
	else
	{
...
QuestMgr::~QuestMgr()
{
	...
	std::unordered_map<uint32, list<QuestRelation*>* >::iterator itr2;
	list<QuestRelation*>::iterator itr3;
	std::unordered_map<uint32, list<QuestAssociation*>* >::iterator itr4;
...
{
	std::unordered_map<uint32, list<QuestAssociation*>* > &associationList = GetQuestAssociationList();
	...
	if(associationList.find(itemId) == associationList.end())
	{
		...
		associationList.insert(std::unordered_map<uint32, list<QuestAssociation*>* >::value_type(itemId, tempList));
	}
	else
	{
